...
vector<int64_t> calcResultsForQueries(const vector<int64_t>& a, const vector<int>& queries)
{
    ...
    // Actually, we don't store *all* positions p - we only store them if factor divides both a[p] and a[p - 1].
    ...
    for (int i = 0; i < n; i++)
    {
        // We *could* just take the set of factors of a[i] and add these (plus the position, i), to
        // positionsWithFactorDecreasing, but this is sadly a little too slow - there can easily
        // be ~1'000'000 factors (with duplicates) over all a[i], and adding these to a map is time-consuming.
        // If the time-limit were 2 seconds instead of 1, it might be better, but ...
        //
        // So instead, we use the fact that the gcd of two numbers will be less than either to trim down the number 
        // of factors we have to deal with - this seems sufficient to pass the testcases.
...
{
    // TODO - proper Editorial-style overview!
    //
    // Some notes: K can be generated by B_1, B_2, ... B_m if and only
    // if gcd(B_1, B_2, ... B_m) divides K.
    //
    // Basic approach, then, is for each possible gcd, find the number of 
    // sub arrays that have that gcd as their gcd.  Do this by essentially,
    // for each position p in a, find the number of subarrays ending at p
    // that have gcd == gcd.
    //
    // Then just use this information along with the above to compute numForK.
