...
int main()
{
    ...
    Coord oxygenTankCoord;
    for (const auto& [coord, contents] : worldMap)
    {
        if (contents == '*')
            oxygenTankCoord = coord;
    }

    // Find shortest time to floodfill (via bfs) the place
    // with oxygen.
    // (Happens when there are no more places to explore).
    vector<Coord> coordsToExplore = { oxygenTankCoord };
    ...
    while (!coordsToExplore.empty())
    {
        ...
        cout << "After " << minutesElapsed << " minutes:" << endl;
        drawWorldMap(oxygenTankCoord, worldMap);
        minutesElapsed++;
    }
    cout << "answer: " << minutesElapsed - 1 << endl;
