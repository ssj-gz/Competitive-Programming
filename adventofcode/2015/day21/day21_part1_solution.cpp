...
void chooseItemsAndFight(vector<Item>::iterator nextItemIter, const vector<Item>& allItems, vector<Item>& chosenItems, const int initialBossHitPoints, const int bossDamage, const int bossArmor, int& minCostToWin)
{
    // Valid?
    int numWeapons = 0;
    int numArmor = 0;
    ...
    if (numWeapons == 1 && numArmor <= 1 && numRings <= 2)
    {
        ...
        {
            std::cout << " can win with cost: " << cost << std::endl;
            if (cost < minCostToWin)
            {
                minCostToWin = cost;
                std::cout << "  new minCostToWin: " << minCostToWin << std::endl;
            }
        }
        else
        {
    ...
    chooseItemsAndFight(std::next(nextItemIter), allItems, chosenItems, initialBossHitPoints, bossDamage, bossArmor, minCostToWin);
...
int main()
{
    ...
    int minCostToWin = std::numeric_limits<int>::max();
//void chooseItemsAndFight(vector<Item>::iterator nextItemIter, const vector<Item>& allItems, vector<Item>& chosenItems, const int initialBossHitPoints, const int bossDamage, const int bossArmor, int& minCostToWin)
    vector<Item> chosenItems;
    chooseItemsAndFight(items.begin(), items, chosenItems, initialBossHitPoints, bossDamage, bossArmor, minCostToWin);
    std::cout << "minCostToWin: " << minCostToWin << std::endl;
