...
#if 0
    for (const auto& [wireName, input_unused] : inputOpForWire)
    {
        std::cout << "signal to " << wireName << ": " << calcSignalOnWire(wireName, inputOpForWire, signalToWireLookup) << std::endl;
    }
#else
    std::cout << "signal to a : " << calcSignalOnWire("a", inputOpForWire, signalToWireLookup) << std::endl;
#endif
