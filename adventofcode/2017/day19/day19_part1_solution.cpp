...
int main()
{
    ...
    string lettersEncountered;
    while (true)
    {
        std::cout << "x: "<< x << " y: " << y << " direction: " << direction << " trackTypeInDirection: " << trackTypeInDirection() << std::endl;
        assert(x >= 0 && x < width);
        assert(y >= 0 && y < height);
        const auto trackType = diagram[x][y];
        if (trackType >= 'A' && trackType <= 'Z')
        {
            lettersEncountered.push_back(trackType);
        }
    ...
    std::cout << "lettersEncountered: " << lettersEncountered << std::endl;
