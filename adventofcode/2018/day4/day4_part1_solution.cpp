...
int main()
{
    ...
    int highestNumMinutesAsleep = -1;
    for (const auto& [guardId, numMinutesAsleep] : guardIdMinutesAsleep)
    {
        std::cout << "guardId " << guardId << " was asleep for " << numMinutesAsleep << std::endl;
        if (numMinutesAsleep > highestNumMinutesAsleep)
        {
            highestNumMinutesAsleep = numMinutesAsleep;
            sleepiestGuardId = guardId;
        }
    }
    map<int, int>& timesAsleepAtMinute = guardIdTimesAsleepAtMinute[sleepiestGuardId];
    int sleepiestMinute = -1;
    int highestTimesAsleepAtMinute = -1;
    for (const auto& [minute, numTimesAsleep] : timesAsleepAtMinute)
    {
        if (numTimesAsleep > highestTimesAsleepAtMinute)
        {
            highestTimesAsleepAtMinute = numTimesAsleep;
