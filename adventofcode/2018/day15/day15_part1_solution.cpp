    ...
    while (true)
    {
        ...
        for (auto unitIter = units.begin(); unitIter != units.end(); unitIter++)
        {
            ...
            if (!foundTarget)
            {
                ...
                std::cout << "result: " << (round - 1) * totalHitPoints << std::endl;
                return 0;
            }
...
#if 0
        const int numElves = std::count_if(units.begin(), units.end(), [](const auto& unit) { return unit.isAlive && unit.type == Unit::Elf; });
        const int numGoblins = std::count_if(units.begin(), units.end(), [](const auto& unit) { return unit.isAlive && unit.type == Unit::Goblin; });
