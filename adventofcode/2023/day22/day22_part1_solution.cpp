...
int main()
{
    ...
    auto printBricks = [&bricks]() {
        map<Coord3D, char> mapContents;
        for (const auto& brick : bricks)
        {
            for (int x = brick.minX(); x <= brick.maxX(); x++)
            {
                for (int y = brick.minY(); y <= brick.maxY(); y++)
                {
                    ...
                    {
                        assert(!mapContents.contains({x, y, z}));
                        mapContents[{x, y, z}] = brick.letter();
                    }
                }
            }
        }
        ...
        for (int y = 0; y <= 9; y++)
        {
            for (int x = 0; x <= 9; x++)
            {
                ...
                for (int z = 0; z <= 9; z++)
                {
                    if (mapContents.contains({x, y, z}))
                    {
                        if (letter != '.')
                            letter = mapContents[{x, y, z}];
                        else
                            letter = '?';
                    }
                }
    ...
    std::cout << " Initial bricks: " << std::endl;
    for (const auto& brick : bricks)
    {
    ...
    std::cout << "Bricks after settling: " << std::endl;
    for (const auto& brick : bricks)
    {
        std::cout << " " << brick << std::endl;
    }
    ...
    for (const auto& brickToRemove : bricks)
    {
        ...
        bool brickCollapsed = false;
        for (const auto& brick : bricksMinusToRemove)
        {
            if (brick.canMoveDown(bricksMinusToRemove))
            {
                std::cout << "Brick " << brickToRemove.letter() << " *cannot* be removed as brick " << brick.letter() << " will fall" << std::endl;
                brickCollapsed = true;
            }
        }
        if (!brickCollapsed)
        {
            std::cout << "Brick " << brickToRemove.letter() << " *can* be removed" << std::endl;
            numSafelyRemovable++;
        }
    }
    std::cout << "# safely removable bricks: " << numSafelyRemovable << std::endl;
