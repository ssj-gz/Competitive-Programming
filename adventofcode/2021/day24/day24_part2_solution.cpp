...
int main()
{
    ...
    const int numFinalInputBlocks = 4;
    ...
    map<int64_t, string> lowestValidDigitsForZ;
    ...
    for (int64_t z = 0; z <= cumulativeDivisionDenom; z++)
    {
        ...
        while (true)
        {
            ...
            if (finalZ == 0)
            {
                ...
                if (!lowestValidDigitsForZ.contains(z) || lowestValidDigitsForZ[z] > testInput)
                {
                    lowestValidDigitsForZ[z] = testInput;
                }
            }
#endif
        }
    ...
    {
        string testInput = string(numFrontInputBlocks, '1');
        while (true)
        {
            int digitIndex = 0;
            while (digitIndex >= 0 && testInput[digitIndex] == '9')
            ...
            if (lowestValidDigitsForZ.contains(finalZ))
            {
                const string full = testInput + lowestValidDigitsForZ[finalZ];
                cout << "opt:" << full << endl;
                found = true;
