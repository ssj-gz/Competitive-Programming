...
int main()
{
    ...
    auto printGrid = [&]()
    {
        ...
        for (int y = minY; y <= maxY; y++)
        {
            for (int x = minX; x <= maxX; x++)
            {
                if (x == SandRockOrigin.x && y == SandRockOrigin.y && (tileTypeOfCoord[SandRockOrigin] == Empty))
                {
                    cout << "+";
                }
                else
    ...
    while (tileTypeOfCoord[SandRockOrigin] != Sand)
    {
    ...
    printGrid();
    std::cout << "# sands before full: " << (numSpawnedSands) << std::endl;
