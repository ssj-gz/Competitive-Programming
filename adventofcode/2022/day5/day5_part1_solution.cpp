...
int main()
{
    ...
    while (std::getline(cin, moveInstructionLine))
    {
        ...
        assert(cratesContents[sourceCrateIndex].size() >= numToMove);
        for (int i = 0; i < numToMove; i++)
        {
            const char crate = cratesContents[sourceCrateIndex].back();
            cratesContents[sourceCrateIndex].pop_back();
            cratesContents[destCrateIndex].push_back(crate);
        }
    }
