...
int main()
{
    ...
    const int64_t usedDiskSpace = calcDirSize(entriesForPath, vector<string>(), dirSizes);
    sort(dirSizes.begin(), dirSizes.end());

    const int64_t totalDiskCapacity = 70'000'000;
    const int64_t requiredDiskSpace = 30'000'000;
    const int64_t amountNeedToFree = requiredDiskSpace - (totalDiskCapacity - usedDiskSpace);
    std::cout << "amountNeedToFree: " << amountNeedToFree << std::endl;

    for (const auto dirSize : dirSizes)
    {
        if (dirSize >= amountNeedToFree)
        {
            std::cout << "Free " << dirSize << std::endl;
            break;
        }
