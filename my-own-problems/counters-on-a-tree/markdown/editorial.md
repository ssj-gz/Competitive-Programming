...
# QUICK EXPLANATION:
This is the game of Nim in disguise: the game $\textit{game}(T, R)$ is equivalent to the game of Nim, where there is a one-to-one correspondence between coins on the board and piles.  For a coin $c$, if $v_c$ is the node containing $c$, then $c$ corresponds to a pile of size $\textit{dist}(R,v)$.  Define

$$
R.\textit{grundy}=\bigoplus_{c}{\textit{dist}(R,v_c)}
$$

where $\oplus$ denotes the xor operator: the this is equivalent to the xor sum of the pile sizes in the Nim game, and by the Sprague-Grundy theorem the second player (Bob) wins $\textit{game}(T, R)$ if and only if $R.\textit{grundy}=0$.

Since $x \oplus x=0$ (where $\oplus$ denotes the xor operator), we see that each pair of coins on the same node contributes $0$ to $R.\textit{grundy}$, so we only care about the _parity_ of the number of coins on each node.  We say that $v$ $\textit{hasCoin}$ if the number of coins on $v$ is odd; thus:
...
We then use Centroid Decomposition in order to _collect_ all contributions of $v$ with $v.\textit{hasCoin}$ and _propagate_ the contributions of the $v$s to all other nodes $R$, essentially calculating $R.\textit{grundy}$ for all $R$, as required.
