...
class DigitVGroupMoveAnim(Animation):
    def __init__(self, digitVGroup, start_pos, end_pos, actual_center, dist_from_center_before_invisible, original_digitMobject, replacement_digitMobject, *args):
    ...
    def interpolate_mobject(self, alpha):
       ...
       self.digitVGroup.move_to(self.start_pos + (self.end_pos - self.start_pos) * (alpha))
...
def create_scroll_digit_to_animation(initial_digitMObject, initial_digit, digit_to_scroll_to, digitMObjectScale = 1):
    ...
    replacement_DigitMObject.move_to(initial_digitMObject.get_center())
    ...
    return DigitVGroupMoveAnim(digitVGroup, digitVGroup.get_center(), digitVGroup.get_center() + amount_to_scroll_by, fully_opaque_at_pos, dist_from_center_before_invisible, initial_digitMObject, replacement_DigitMObject)
