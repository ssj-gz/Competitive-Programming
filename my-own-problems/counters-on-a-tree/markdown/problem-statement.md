Bob and Alice are playing a game with Counters on a Tree $T$, which has $N$ nodes each labelled with a distinct number from $1$ to $N$.  For a node $R$ ($1 \leq R \leq N)$, define $\textit{game}(T, R)$ as the game with the following rules:
