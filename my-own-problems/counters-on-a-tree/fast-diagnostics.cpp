...
void winningNodeInfo(Node* rootNode)
{
    int largestDistWithCoin = -1;
    int dist = 0;

    set<Node*> visited = { rootNode };
    vector<Node*> toExplore = { rootNode };

    int totalExplored = 0;
    int numDistsWithCoin = 0;
    ...
    cout << "dist: " << dist << " largestDistWithCoin: " << largestDistWithCoin << " numDistsWithCoin: " << numDistsWithCoin << endl;
    //cout << "totalExplored: " << totalExplored << endl;
...
int main(int argc, char* argv[])
{
    ...
    for (int t = 0; t < numTestcases; t++)
    {
        ...
        for (auto edgeNum = 0; edgeNum < numNodes - 1; edgeNum++)
        {
            ...
            nodes[node1Index].originalNeighbours.push_back(&(nodes[node2Index]));
            nodes[node2Index].originalNeighbours.push_back(&(nodes[node1Index]));
        }
        ...
        for (auto node : bobWinNodes)
        {
            //cout << "largestDistWithCoin: " << largestDistWithCoin(node) << endl;
            winningNodeInfo(node);
        }
    }
}
