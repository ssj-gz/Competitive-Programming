...
struct Node
{
    ...
    vector<Node*> originalNeighbours;
...
void distanceBetweeNodesHelper(Node* currentNode, Node* parentNode, int distanceTravelled, Node* targetNode, int& distance)
{
    ...
    for (auto child : currentNode->neighbours)
    {
        if (child == parentNode)
            continue;
        distanceBetweeNodesHelper(child, currentNode, distanceTravelled + 1, targetNode, distance);
    }
}

int distanceBetweeNodes(Node* node1, Node* node2)
{
    int distance = -1;
    distanceBetweeNodesHelper(node1, nullptr, 0, node2, distance);
    assert(distance != -1);
    return distance;
}
    ...
    for (int t = 0; t < numTestcases; t++)
    {
        ...
        for (auto& node : nodes)
        {
            ...
            if (node.grundyNumberIfRoot == 0)
            {
                ...
                numBobWins++;
                cout << "Winning id: " << nodeId << endl;
                bobWinNodes.push_back(&node);
            }
            powerOf2 = (powerOf2 * 2) % MOD;
            nodeId++;
        }
        //cout << result << endl;
        cout << numBobWins << endl;
        for (int i = 0; i < bobWinNodes.size(); i++)
        {
            for (int j = i + 1; j < bobWinNodes.size(); j++)
            {
                cout << "Distance from Bob Win Node # " << i << " to Bob Win Node # " << j << " : " << distanceBetweeNodes(bobWinNodes[i], bobWinNodes[j]) << endl;
            }
        }
    }
}
