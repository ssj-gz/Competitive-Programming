...
// 
// Cheat Solution to: https://www.codechef.com/problems/MVCN2TST
//
// Hopefully won't be able to pass all testcases!
//
#include <iostream>
#include <vector>
#include <deque>
...
int64_t calcFinalDecryptionKey(vector<Node>& nodes, const vector<int64_t>& encryptedQueries)
{
    ...
    for (const auto encryptedQuery : encryptedQueries)
    {
        ...
        const int cacheNumDescendantsCutoff = 1000;
        const int numDescendantsAtNewParentHeight = nodesAtHeightLookup[newParentHeight].size() - (numNonDescendantsToLeft + numNonDescendantsToRight);
