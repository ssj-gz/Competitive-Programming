        ...
        if (printUndoStack)
        {
            ...
            for (int undoStackIndex = 0; undoStackIndex < undoStackDocuments.size(); undoStackIndex++)
            {
                ...
                auto flushLine = [&]()
                {
                    ...
                    {
                        const auto undoStackPointerText = "SP = " + to_string(undoStackPointer + 2);
            ...
            case Query::IsRangeFormatted:
                {
                    ...
                    if (queryAnswer == -1)
                    {
                        ...
                        cout << "The queried position is not part of a formatted range; $A = 3141592$." << endl;
                    }
                    else
                        cout << "The queried position is part of a formatted range and the number of ``X``s is this range is " << queryAnswer << "; so $A = " << queryAnswer << "$." << endl;
