...
#ifdef DIAGNOSTICS
                    cout << "Need to insert " << "$c = d \\oplus " << query.encryptedArgument2 << " = " << decryptionKey << " \\oplus " << query.encryptedArgument2 << " = " << numToInsert << "$ non-formatting chars at position $p=" << decryptionKey << "\\oplus " << query.encryptedArgument2 << " = " << (insertionPos + 1) << "$." << endl;
                    cout << "```" << endl;
                    showStatus(true, false, false);
...
#ifdef DIAGNOSTICS
                    cout << "Need to move forward the undo stack pointer by $" << decryptionKey << "\\oplus" << query.encryptedArgument << " = " << numToRedo << "$ places.\n" << endl;
#endif
