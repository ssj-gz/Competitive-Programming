...
ModNum calcNumPSequences(int N, int P)
{
    ...
    vector<ModNum> firstEndingOnDivChangeIndex(divisionChangesOfP.size() + 1, 0);
    for (int i = 0; i < divisionChangesOfP.size(); i++)
    {
        // One P-Sequence of length 1.
        firstEndingOnDivChangeIndex[i] = 1;
    }
    for (int i = 1; i < N; i++)
    {
        ...
        nextFirstNEndingOnDivChangeIndex[divChangeIndex] = firstEndingOnDivChangeIndex[inverseChangeIndex];
        ...
        ModNum sumOfPreviousPSequences = firstEndingOnDivChangeIndex[inverseChangeIndex];
        ...
        while (divChangeIndex >= 1)
        {
            ...
            while (inverseChangeIndex + 1 < divisionChangesOfP.size() && divisionChangesOfP[inverseChangeIndex + 1] <= numOfPSequencesNeededInSum)
            {
                ...
                sumOfPreviousPSequences += 
                    ...
                    (divisionChangesOfP[inverseChangeIndex] - numPSequencesSummed - 1) * firstEndingOnDivChangeIndex[inverseChangeIndex - 1]
                    // ... plus the new divisionChangesOfP[inverseChangeIndex].
                    + firstEndingOnDivChangeIndex[inverseChangeIndex];
                numPSequencesSummed = divisionChangesOfP[inverseChangeIndex];
            }
            // Add the remaining divisionChangesOfP[inverseChangeIndex].
            sumOfPreviousPSequences += (numOfPSequencesNeededInSum - divisionChangesOfP[inverseChangeIndex]) * firstEndingOnDivChangeIndex[inverseChangeIndex];
        ...
        firstEndingOnDivChangeIndex = nextFirstNEndingOnDivChangeIndex;
    }
    ...
    {
        result += firstEndingOnDivChangeIndex[r] * (divisionChangesOfP[r + 1] - divisionChangesOfP[r]);
    }
    // Scoop up the remainder (from the last divisionChangesOfP up to P).
    const auto numRemainingUpToP = (P - divisionChangesOfP.back() + 1);
    result += firstEndingOnDivChangeIndex[divisionChangesOfP.size() - 1]  * numRemainingUpToP;
...
int main(int argc, char* argv[])
{
    ...
    // hopefully be obvious that if start d at P and work downwards, the sum is easily
    ...
    // list of values P / divisionChangesOfP[x] != P / (divisionChangesOfP[x] - 1) - I intuited,
    ...
    // in O(N x sqrt(P)), which is much more manageable :)
    //
    ...
    // O(P) would be too big a vector to allocate, so we compute a sparse versoin of firstNEndingOnP which
    // we call firstEndingOnDivChangeIndex.  The fiddliness comes from keeping sumOfPreviousPSequences
