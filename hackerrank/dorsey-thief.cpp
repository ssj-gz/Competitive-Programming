...
int64_t findBestPriceForSellingAllGold(int N, int X, const vector<int>& v, const vector<int>& a)
{
    ...
    for (const auto amountOfGoldToSell : distinctGoldAmounts)
    {
        ...
        for (int allGoldSoldSorFar = 0; allGoldSoldSorFar <= X; allGoldSoldSorFar++)
        {
            if (bestPriceForSoldGold[allGoldSoldSorFar] == -1)
                continue;
            ...
            for (int numBuyers = 1; numBuyers <= totalBuyersForGoldAmount; numBuyers++)
            {
                ...
                if (allGoldSoldSorFar + amountOfSoldGoldAmount > X)
                {
                    // Can't sell more gold that we started with!
                    break;
                }
                bestPriceForNextSoldGold[allGoldSoldSorFar + amountOfSoldGoldAmount] = max(bestPriceForNextSoldGold[allGoldSoldSorFar + amountOfSoldGoldAmount], 
                                                                                      bestPriceForSoldGold[allGoldSoldSorFar] + priceOfSoldGoldAmount);
            }
        }
...
int main()
{
    ...
    // units of gold that buy, and that there will be at most X + 1 such groups, and
    // we can easily determine how many passengers are in a given group.  Let distinctGoldAmounts
    // be the amount of gold in each of these sets of groups.
    //
    ...
    // over all amounts of sold gold g and then, for each possible subset of buyers of distinctGoldAmounts[n+1],
    // updating bestPriceForSoldGold[n+1] in the obvious way i.e.
    //
    //    for allGoldSoldSorFar = 0, 1, 2, ... , X:
    ...
    //          bestPriceForSoldGold[n+1][allGoldSoldSorFar + amountOfSoldGoldAmount] 
    //            = max(bestPriceForSoldGold[n+1][allGoldSoldSorFar + amountOfSoldGoldAmount],
    //                  bestPriceForSoldGold[n][allGoldSoldSorFar] + priceOfSoldGoldAmount);
    // 
    ...
    // pick the x passengers who buy distinctGoldAmounts[n+1] for the highest price.  Thus, after grouping our
    ...
    // Doing this over all N, an upper bound for the complexity is O(N * X) - the
    //
    //    for n = 0, 1, ... , N:
    //       <snip>
    //       for numBuyers = 0, 1, ..., total that buy distinctGoldAmounts[n+1]:
    //
    // is the same as iterating over all passengers, and for each passenger we loop over X + 1 values of allGoldSoldSorFar.
    //
    ...
    // increasing) etc.  This diminishes very quickly, and we end up performing (asymptotically?) fewer computations that
    // O(N * X), though I can't quite calculate the tighter asymptotic bound.  
    //
    // Anyway, the simple step of avoid computing bestPriceForSoldGold[n+1][g] for g > X, which we need to do for correctness,
